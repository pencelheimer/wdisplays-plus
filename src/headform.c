...
static void wd_head_form_class_init(WdHeadFormClass *class) {
  ...
  gtk_widget_class_bind_template_child_private(widget_class, WdHeadForm, kanshi_name);
...
void wd_head_form_update(WdHeadForm *form, const struct wd_head *head, enum wd_head_fields fields) {
  ...
  if (fields & (WD_FIELD_MAKE | WD_FIELD_MODEL | WD_FIELD_SERIAL_NUMBER)) {
    g_autofree gchar *kanshi_str = g_strdup_printf("%s %s %s", head->make ? head->make : "Unknown",
                                                   head->model ? head->model : "Unknown",
                                                   head->serial_number ? head->serial_number : "Unknown");
    gtk_label_set_text(GTK_LABEL(priv->kanshi_name), kanshi_str);
  }
  if (fields & WD_FIELD_PHYSICAL_SIZE) {
    g_autofree gchar *physical_str = g_strdup_printf("%dmm × %dmm", head->phys_width_mm, head->phys_height_mm);
