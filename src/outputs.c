...
static void config_handle_cancelled(void *data, struct zwlr_output_configuration_v1 *config) {
  ...
  zwlr_output_configuration_v1_destroy(config);
  wd_ui_apply_done(pending->state, NULL);
  wd_ui_show_error(pending->state, "The display configuration was modified by the server before updates were processed. "
...
static void head_handle_physical_size(void *data, struct zwlr_output_head_v1 *wlr_head, int32_t width, int32_t height) {
  struct wd_head *head = data;
  head->phys_width_mm  = width;
  head->phys_height_mm = height;
...
static void head_handle_make(void *data, struct zwlr_output_head_v1 *wlr_head, const char *make) {
  struct wd_head *head = data;
  head->make           = strdup(make);
  wd_ui_reset_head(head, WD_FIELD_MAKE);
}

static void head_handle_model(void *data, struct zwlr_output_head_v1 *wlr_head, const char *model) {
  struct wd_head *head = data;
  head->model          = strdup(model);
  wd_ui_reset_head(head, WD_FIELD_MODEL);
}

static void head_handle_serial_number(void *data, struct zwlr_output_head_v1 *wlr_head, const char *serial_number) {
  struct wd_head *head = data;
  head->serial_number  = strdup(serial_number);
  wd_ui_reset_head(head, WD_FIELD_SERIAL_NUMBER);
}

static void head_handle_adaptive_sync(void *data, struct zwlr_output_head_v1 *wlr_head, uint32_t state) {
  struct wd_head *head = data;
...
static void registry_handle_global(void *data, struct wl_registry *registry, uint32_t name, const char *interface,
  ...
  if (strcmp(interface, zwlr_output_manager_v1_interface.name) == 0) {
    printf("%u", version);
    state->output_manager = wl_registry_bind(registry, name, &zwlr_output_manager_v1_interface, version >= 4 ? 4 : 2);
